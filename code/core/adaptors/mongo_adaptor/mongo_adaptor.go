...
type Config struct {
	...
	SharedDBName         string
	SharedPasswordConfig config.PasswordConfig

	AppMongoURL       string
	AppDBName         string
	AppPasswordConfig config.PasswordConfig
}

func (cfg *Config) OnConfigLoaded() {

	sharedPasswd, err := cfg.SharedPasswordConfig.GetPassword()
	if err != nil {
		logger.LogError("error getting shared mongodb password" +
			"|error=" + err.Error())
	} else {
		cfg.SharedMongoURL = strings.Replace(cfg.SharedMongoURL, "$password", sharedPasswd, -1)
	}

	appPasswd, err := cfg.AppPasswordConfig.GetPassword()
	if err != nil {
		logger.LogError("error getting app mongodb password" +
			"|error=" + err.Error())
	} else {
		cfg.AppMongoURL = strings.Replace(cfg.AppMongoURL, "$password", appPasswd, -1)
	}
}
...
func getMongoCollection(dbSpace DBSpace, collectionName string) (*mongo.Collection, error) {
	...
	switch dbSpace {
	case SHARED_DB:
		database = _sharedMongoClient.Database(_sharedDBName)
	case APP_DB:
		database = _appMongoClient.Database(_appDBName)
	}
	if database == nil {
